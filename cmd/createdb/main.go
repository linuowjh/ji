...
func main() {
	...
	log.Printf("✅ 数据库 '%s' 创建成功（或已存在）", database)

	// 验证数据库是否存在
	var dbName string
	err = db.QueryRow("SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = ?", database).Scan(&dbName)
	if err != nil {
		log.Fatalf("验证数据库失败: %v", err)
	}

	log.Printf("✅ 验证成功: 数据库 '%s' 已存在", dbName)
}
