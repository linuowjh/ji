...
type Memorial struct {
	...
	DeceasedName   string         `json:"deceased_name" gorm:"type:varchar(50);not null;comment:逝者姓名"`
	BirthDate      *time.Time     `json:"birth_date" gorm:"comment:出生日期"`
	DeathDate      *time.Time     `json:"death_date" gorm:"comment:逝世日期"`
	Biography      string         `json:"biography" gorm:"type:text;comment:生平简介"`
	AvatarURL      string         `json:"avatar_url" gorm:"type:varchar(255);comment:头像URL"`
	ThemeStyle     string         `json:"theme_style" gorm:"type:varchar(50);default:traditional;comment:主题风格"`
	TombstoneStyle string         `json:"tombstone_style" gorm:"type:varchar(50);default:marble;comment:墓碑样式"`
	Epitaph        string         `json:"epitaph" gorm:"type:text;comment:墓志铭"`
	PrivacyLevel   int            `json:"privacy_level" gorm:"default:1;comment:隐私级别:1家族可见 2私密"`
...
type MediaFile struct {
	...
	FileURL     string         `json:"file_url" gorm:"type:varchar(255);not null;comment:文件URL"`
